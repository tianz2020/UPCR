...
class Upcrtopic(nn.Module):
    def __init__(self,vocab:Vocab,user_cont,n_layers=6,p_layers=3,
        ...
        self.word_vocab, self.word_len, self.topic_vocab, self.topic_len = vocab.get_vocab(task='topic')
    ...
    def forward(self,user_id,all_topic, all_topic_len,context, context_len,tp_path, tp_path_len,ar_gth, ar_gth_len,
                related_topics, related_topics_len,final,pv_m,mode='train'):
        ...
        if mode == 'train':
            ...
            q_m, q_m_gumbel = self.q_mt.forward(context=context, context_len=context_len,pv_m=pv_m,
                                                ...
                                                tp_path=tp_path,tp_path_len=tp_path_len)
            ar = self.action.forward(m=q_m_gumbel,l=q_l_gumbel,context=context,context_len=context_len,ar_gth=ar_gth,ar_gth_len=ar_gth_len,
                                     tp_path=tp_path,tp_path_len=tp_path_len,related_topics=related_topics,related_topics_len=related_topics_len,
                                     mode='train')
            ...
            ar,ar_probs = self.action.forward(m=p_m,l=p_l,context=context,context_len=context_len,ar_gth=ar_gth,ar_gth_len=ar_gth_len,
                                     tp_path=tp_path,tp_path_len=tp_path_len,related_topics=related_topics, related_topics_len=related_topics_len,
                                     mode='test')
...
class Engine():
    ...
    def train(self,train_set, test_set):
        for e in range(op.epoch):
            ...
            pv_m = get_default_tensor([op.batch_size, op.preference_num, self.model.topic_len], torch.float,pad_idx=self.topic_pad_idx)
            self.optimizer.zero_grad()
            for index,input in enumerate(train_loader):
                if input[0].size(0) != op.batch_size:
                    break
                id, context_idx, context_len, state_U, state_U_len, related_topics, related_topics_len, \
                a_R, a_R_len, all_topic, all_topic_len,final = input
                p_l, q_l,  p_m, q_m, ar,  m= self.model.forward(user_id=id,all_topic=all_topic,all_topic_len=all_topic_len,context=context_idx, context_len=context_len,
                                                                tp_path=state_U,tp_path_len=state_U_len,ar_gth=a_R, ar_gth_len=a_R_len,related_topics=related_topics,
                                                                related_topics_len=related_topics_len,final=final,pv_m=pv_m)
                ...
                loss = 0.5 * kl_l  + nll_ar + reg_loss + 0.5 * kl_m
    ...
    def test(self,test_set):
        ...
        pv_m = get_default_tensor([op.batch_size, op.preference_num, self.model.topic_len], torch.float,pad_idx=self.model.topic_pad_idx)
        with torch.no_grad():
            for index,data in enumerate(dataloader):
                if data[0].size(0) != op.batch_size:
                    break
                id, context_idx, context_len, state_U, state_U_len, related_topics, related_topics_len, \
                a_R, a_R_len, all_topic, all_topic_len, final = data
                ar, ar_probs, m , l = self.model.forward(user_id=id,all_topic=all_topic,all_topic_len=all_topic_len,context=context_idx, context_len=context_len,
                                                         tp_path=state_U,tp_path_len=state_U_len,ar_gth=a_R, ar_gth_len=a_R_len,related_topics=related_topics,
                                                         related_topics_len=related_topics_len,final=final,pv_m=pv_m,mode='test')
