...
class PriorPreference(nn.Module):
    ...
    def forward(self,context,context_len,pv_m,pv_m_mask,tp_path,tp_path_len,
                movie_path,movie_path_len,relations,relations_len):
        '''
        pv_ru           [B,Lc]
        pv_ru_hidden    [B,Lc,H]
        pv_ru_mask      [B,1,Lc]
        ...
        # movie path
        movie_path_mask = Tools.get_mask_via_len(movie_path_len,op.movie_path_len)
        movie_path_hidden = self.m_encoder(movie_path,movie_path_mask)

        # movie path 对应的topic
        mv_path_topics_hidden, mv_path_topics_mask, mv_path_topics = get_movie_rep(movie_path, movie_path_mask, self.p_encoder,
                                                 relations, relations_len,op.movie_path_len)
        mv_path_topics = one_hot_scatter(mv_path_topics,self.n_topic_vocab)

        # union
        src_hiddens = torch.cat([pv_m_hidden,context_hidden,mv_path_topics_hidden,movie_path_hidden], 1)  # [B,L_m+L_c,H]
        src_mask = torch.cat([pv_m_mask,context_mask,mv_path_topics_mask,movie_path_mask], 2)  # [B,1,L_m+L_c]
        ...
        for _ in range(op.preference_num):
            ...
            single_step_prob = self.proj(dec_out=dec_output,context=context,src_hidden=src_hiddens,
                                         src_mask=src_mask,pv_m=pv_m,tp_path=mv_path_topics) # [B,1,V]
    ...
    def proj(self,dec_out,context,src_hidden,src_mask,pv_m,tp_path):
        '''
        src_hiddens = torch.cat([
        pv_m_hidden,
        context_hidden,
        mv_path_topics_hidden,
        movie_path_hidden], 1)  # [B,L_m+L_c,H]
        '''
        # generation  [B,1,V]
        ...
        # copy
        copy_hidden = src_hidden[:,0:-op.movie_path_len,:]
        copy_mask = src_mask[:,:,0:-op.movie_path_len]
        copy_logit = torch.bmm(dec_out, copy_hidden.permute(0, 2, 1))
        copy_logit = copy_logit.masked_fill((copy_mask == 0).expand(-1, L_s, -1), -1e9)
        ...
        # copy from tp
        copy_tp_prob = probs[:, :,self.n_topic_vocab + op.preference_num+op.context_max_len: ]
        copy_tp_prob = torch.bmm(copy_tp_prob,tp_path)

        # copy from context  第i维代表复制context中第i个词的概率
        copy_context_prob = probs[:,:,self.n_topic_vocab + op.preference_num:self.n_topic_vocab + op.preference_num+op.context_max_len]
        ...
        probs = gen_prob + copy_context_prob + copy_pv_m_prob + copy_tp_prob
        return probs
...
class PosteriorPreference(nn.Module):
    ...
    def forward(self,context,context_len,pv_m,pv_m_mask,ar_gth,ar_gth_len,
                tp_path,tp_path_len,movie_path,movie_path_len,relations,relations_len):
        '''
        pv_ru           [B,Lc]
        pv_ru_hidden    [B,Lc,H]
        pv_ru_mask      [B,1,Lc]
        ...
        ar_hidden = self.m_encoder(ar_gth, ar_mask)
        ar_topic_hidden,ar_topic_mask,ar_topic = get_movie_rep(ar_gth,ar_mask,self.p_encoder,relations,relations_len,1)
        ...
        # tp_hidden = self.encoder(tp_path, tp_mask)
        ...
        src_hiddens = torch.cat([pv_m_hidden,context_hidden,mv_path_topic_hidden,ar_topic_hidden,movie_hidden,ar_hidden], 1)
        src_mask = torch.cat([pv_m_mask,context_mask,mv_path_mask,ar_topic_mask,movie_mask,ar_mask], 2)
        ...
        for _ in range(op.preference_num):
            ...
            single_step_prob = self.proj(dec_out=dec_output,src_hidden=src_hiddens,src_mask=src_mask,
                                         pv_m=pv_m,context=context,tp=mv_path_topic)  # [B,1,V]
            if self.training:
                single_step_gumbel_word = self.gs.forward(single_step_prob, self.ts.step_on(),normed=True)
                if seq_gen_prob is not None:
                    seq_gen_prob = torch.cat([seq_gen_prob, single_step_prob], 1)
    ...
    def proj(self, dec_out,  src_hidden, src_mask, pv_m , context ,tp):
        ...
        hidden_no_At = src_hidden[:,0:- (int(op.action_num/2) + op.movie_path_len + 3),:]
        mask_no_At = src_mask[:,:,0:- ( int(op.action_num/2) + op.movie_path_len + 3)]
        ...
        copy_tp_path_prob = probs[:, :,self.n_topic_vocab + op.preference_num + op.context_max_len:]
        copy_tp_path_prob = torch.bmm(copy_tp_path_prob, tp)

        probs = gen_prob + copy_context_prob + copy_pv_m_prob + copy_tp_path_prob
        return probs
