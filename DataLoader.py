...
class DataLoader():
    ...
    def process(self,conversation):
        ...
        profile = self.user2profile[conversation[0]]
        profile, profile_len = clip_pad_sentence(profile,option.profile_num)
        profile = self.vocab.topic2index(profile)
        id = int(conversation[0])
        contexts = conversation[-1]

        conv, conv_len = clip_pad_context(contexts,option.conv_max_len)
        conv_idx = self.vocab.word2index(conv)

        utterances = conversation[1:-1]
        uttr_len = len(utterances)

        # profile的后验的条件
        all_topics = []
        for topic in utterances[-1][1]:
        ...
        for i in range(2,uttr_len,2):
            ...
            # intention
            action_U = Seeker[2]
            a_U, a_U_len = clip_pad_sentence(action_U, option.action_num)

            # related_topics
            related_topics = self.get_related_topics(action_U,option.relation_num)
            related_topics, related_topics_len = clip_pad_sentence(related_topics,option.relation_num)


            # all topic
            all_topic, all_topic_len = clip_pad_sentence(all_topics,option.state_num)
            ...
            session_segs.append([id,
                                 ...
                                 # related_topics,related_topics_len,adj,
                                 0])
    ...
    #             if state == t1:
    #                 if t2 > 2582 :  # movie
    #                     if t2 in related_topics:
    #                         related_topics.remove(t2)
    #                         related_topics.append(t2)
    #                     else:
    #                         related_topics.append(t2)
    #                 else:
    #                     if t2 in prepared_topics:
    #                         prepared_topics.remove(t2)
    #                         prepared_topics.append(t2)
    #                     else:
    #                         prepared_topics.append(t2)
    #             elif state == t2:
    #                 if t1 > 2582 :
    #                     if t1 in related_topics:
    #                         related_topics.remove(t1)
    #                         related_topics.append(t1)
    #                     else:
    #                         related_topics.append(t1)
    #                 else:
    #                     if t1 in prepared_topics:
    #                         prepared_topics.remove(t1)
    #                         prepared_topics.append(t1)
    #                     else:
    #                         prepared_topics.append(t1)
    #
    #     for state in states:
    #         if state not in related_topics and state!=2:
    #             related_topics.append(state)
    #         if state in prepared_topics:
    #             prepared_topics.remove(state)
    #
    #     if len(related_topics) >= 100:
    #         related_topics = related_topics[-100:]
    #
    #         adj = self.adj[:,related_topics]
    #         adj = adj[related_topics,:]
    #         adj = adj.reshape(-1)
    #         adj = [int(i) for i in adj ]
    #         return related_topics , 100 , adj
    #     else:
    #         prepared_topics = prepared_topics[-(100-len(related_topics)) :]
    #         related_topics = related_topics + prepared_topics
    #         related_topics_len = len(related_topics)
    #         if len(related_topics) < 100:
    #             related_topics = related_topics + [2] * (100-len(related_topics))
    #         adj = self.adj[:, related_topics]
    #         adj = adj[related_topics, :]
    #         adj = adj.reshape(-1)
    #         adj = [int(i) for i in adj ]
    #         return related_topics , related_topics_len, adj
    #
    # def get_adj(self):
    #     # relations = []
    #     # with open('./dataset/relations.csv', 'r') as f:
    #     #     reader = csv.reader(f)
    #     #     for relation in reader:
    #     #         t1 = int(relation[0])
    #     #         t2 = int(relation[1])
    #     #         relations.append([t1,t2])
    #     topic_num = self.vocab.topic_num()
    #     adj = torch.zeros([topic_num,topic_num],dtype=torch.int)
    #     relations = torch.tensor(self.graph)
    #     adj[relations[:,0],relations[:,1]] = 1
    #     adj[relations[:,1],relations[:,0]] = 1
    #     return adj
    #
    # def get_graph(self):
    #     graph = []
    ...
    def get_related_topics(self, action_U, relation_num):
        related_topics = []
        a_len = len(action_U)
