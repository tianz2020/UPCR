...
class ExcrsTopic(nn.Module):
    ...
    def cal_tp_hidden(self,tp,tp_len,word2token):
        ...
        tp_token_hidden = tp_token_hidden[0]  # [B,L,H]
        tp_word_hidden = None
        for b in range(op.batch_size):
            tp_hiddens = None
            i = 0
            token_hidden = tp_token_hidden[b, :, :]  # [L,H]
            w2t = word2token[b, :]  # [L]
            for length in w2t:
                if length == 1:
                    i+=length
...
class EngineTopic():
    def __init__(self,model:torch.nn.Module,
        ...
        # 第二阶段一起训练
        # bert_params = list(map(id, self.model.main_tfr_encoder.parameters()))
        ...
        #                                    {'params': self.model.main_tfr_encoder.parameters(), 'lr': 0.6 * lr}],
