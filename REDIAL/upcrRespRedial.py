...
class UPCR(nn.Module):
    def __init__(self,vocab:Vocab,user_cont,n_layers=6,
                 d_word_vec=512,d_model=512, d_inner=2048,beam_width=1,
                 n_head=8, d_k=64, d_v=64, dropout=0.1):
        super(UPCR, self).__init__()
        self.vocab = vocab
    ...
    def train(self,train_set,valid_set,test_set):
        for e in range(op.epoch):
            ...
            for index,input in enumerate(train_loader):
                ...
                resp_gen = self.model.forward(context=context_idx, context_len=context_len,
                ...
                if self.global_step % op.gradient_stack == 0:
                    ...
                    self.optimizer.zero_grad()
                self.global_step += 1
            self.test(valid_set,test_set,'valid')
            self.test(test_set,test_set,'test')

        print("train finished...")

    def test(self,valid_set,test_set,mode):
        ...
        if mode == "valid":
            print(" valid ")
            dataloader = DataLoaderResp(valid_set,self.vocab)
        else:
            print(" test ")
        ...
        with torch.no_grad():
            for index,data in enumerate(dataloader):
                ...
                resp, resp_len, \
                final = data
                resp_gen,probs = self.model.forward(context=context_idx, context_len=context_len,
                                                    tp_path=state_U,tp_path_len=state_U_len,
                                                    ar_gth=a_R, ar_gth_len=a_R_len,
