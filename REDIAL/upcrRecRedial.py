...
from ProfileRecRedial import PriorProfile,PosteriorProfile
...
class Engine():
    ...
    def train(self,train_set,valid_set,test_set):
        for e in range(op.epoch):
            ...
            for index,input in enumerate(train_loader):
                ...
                loss = 0.1 * kl_m + nll_ar + reg_loss + 0.1 * kl_l
                ...
                self.global_step += 1
                pv_m = m.detach()
            self.test(valid_set,test_set,'test')
    ...
    def test(self,valid_set,test_set,mode):
        ...
        if mode == "valid":
            print(" valid ")
            dataloader = DataLoaderRec(valid_set,self.vocab)
        else:
            print(" test ")
