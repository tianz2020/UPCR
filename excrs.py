...
class Excrs(nn.Module):
    def __init__(self,vocab:Vocab,user_cont,n_layers=6,p_layers=3,
        ...
        self.m_tfr_encoder4p = Encoder(
            n_src_vocab=self.movie_len, n_position=100,
            d_word_vec=d_word_vec, d_model=d_model, d_inner=d_inner,
            n_layers=p_layers, n_head=n_head, d_k=d_k, d_v=d_v,
            pad_idx=self.topic_pad_idx, dropout=dropout, scale_emb=False,
            word_emb=self.movie_emb
        )
        ...
        self.a_tfr_decoder = Decoder(
            n_trg_vocab=self.movie_len, n_position=op.action_num,
            d_word_vec=d_word_vec, d_model=d_model, d_inner=d_inner,
            n_layers=p_layers, n_head=n_head, d_k=d_k, d_v=d_v,
            pad_idx=self.topic_pad_idx, dropout=dropout, scale_emb=False,
            word_emb=self.movie_emb
    ...
    def forward(self,
        ...
        if mode == 'train':
            ...
            q_l, q_l_gumbel = self.q_l.forward(id=user_id, topics=all_topic, topics_len=all_topic_len,
                                               movies=all_movie,movies_len=all_movie_len,relations=relations,
                                               relations_len=relations_len)
            ...
            p_m, p_m_gumbel = self.p_mt.forward(context=context, context_len=context_len,pv_m=pv_m,pv_m_mask=pv_m_mask,
                                                tp_path=tp_path,tp_path_len=tp_path_len,movie_path=movie_path,
                                                movie_path_len=movie_path_len,relations=relations,relations_len=relations_len)
            ...
            ar = self.action.forward(m=q_m_gumbel,
                                     ...
                                     state=tp_path,state_len=tp_path_len,
                                     movie_path=movie_path,movie_path_len=movie_path_len,
                                     related_topics=related_topics,related_topics_len=related_topics_len,
                                     relations=relations, relations_len=relations_len,
                                     mode='train')
        ...
        for e in range(op.epoch):
            ...
            for index,input in enumerate(train_loader):
                ...
                p_l, q_l, au, p_m, q_m, ar, resp, m= self.model.forward(user_id=id,
                                                                        ...
                                                                        all_movie=all_movie,all_movie_len=all_movie_len,
        ...
        with torch.no_grad():
            for index,data in enumerate(dataloader):
                ...
                # if index % 400 == 0:
                    ...
                    # self.logger.info("----------------------------------------------------")
                    # print("ar: {} , ar_gth: {} ".format(self.model.topictensor2nl(ar),self.model.topictensor2nl(a_R)))
                    # sys.stdout.flush()
                    # self.logger.info("----------------------------------------------------")
